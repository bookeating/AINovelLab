...
注意：脱水处理需要使用AI API，支持Gemini API和OpenAI API格式，请确保api_keys.json文件中已配置有效的API密钥。您可以在"API测试"页面测试配置的API是否可用。"""
        )
...
6. 等待处理完成"""
        ...
        notes_card = CardWidget(
            "注意事项", 
            "settings",
            """1. API密钥配置：脱水处理支持多种API格式：
   - Gemini API（包括官方API和第三方API）
   - OpenAI API格式（支持官方API和兼容的第三方API）
2. API测试：使用"API测试"页面可以测试配置的API是否可用
3. 处理时间：脱水处理可能需要较长时间，特别是对于内容较多的小说
4. 章节范围：对于长篇小说，建议分批处理，每次选择一定范围的章节进行脱水
5. 文件命名：本工具依赖于特定的文件命名格式来识别章节顺序，请勿手动修改生成的文件名
6. 数据备份：处理前建议备份原始EPUB文件，以防止意外情况导致数据丢失"""
        )
